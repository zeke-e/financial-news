...
def send_email(analysis, recipient_email):
    ...
    try:
        ...
        sender_email = credentials_dict.get('client_email')
        print(f"✓ Credentials loaded, service account: {sender_email}")
        
        credentials = Credentials.from_service_account_info(
            credentials_dict,
            scopes=['https://www.googleapis.com/auth/gmail.send']
        )
        print("✓ Credentials created with gmail.send scope")
        
        # Build Gmail service
        service = build('gmail', 'v1', credentials=credentials)
        print("✓ Gmail service built")
        
        # Create simple text message (not HTML)
        message_text = f"""Daily Financial News Analysis
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

{analysis}

---
This analysis was generated by Claude analyzing the day's financial news.
"""
